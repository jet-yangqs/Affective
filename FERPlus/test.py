...
def test(impath):
    ...
    image = np.array(image).reshape((1, size, size))
    L = sess.run(softmax, feed_dict={x: image, keep_prob:1.0})
    argmax = 0
    max = 0.0
    for cnt in range(1, len(L[0])):
       if L[0][cnt] > max:
          max = L[0][cnt]
          argmax = cnt
    return np.argmax(L[0]), L[0], argmax, cnt 
...
with tf.Session() as sess:
    ...
    try:
        ...
        pass
    testimages = sys.argv[1].strip().split(',')
    for testimage in testimages:
        result = test(testimage.strip())
        print(result)
